...
class CommonSpec extends AnyFlatSpec {
  ...
  "ProcessErrorType" should "contain the correct enums" in {
    ...
    val expectedValues =
      List("CLIENT_DATA_LOAD", "ENCODING", "EVENT", "FILE_NAME", "JSON", "MALWARE_SCAN", "MATCH_ID", "OBJECT_COUNT", "OBJECT_KEY", "OBJECT_SIZE", "S3", "STATE", "UPLOAD")

    processErrorType.values.size shouldBe 13
  ...
  "ProcessErrorValue" should "contain the correct enums" in {
    ...
    val expectedValues = List("FAILURE", "FOLDER_ONLY", "INCORRECT", "INVALID", "MISMATCH", "READ_ERROR", "THREAT_FOUND", "TOO_BIG", "TOO_SMALL")
