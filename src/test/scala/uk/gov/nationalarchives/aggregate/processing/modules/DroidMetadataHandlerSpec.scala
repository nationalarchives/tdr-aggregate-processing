...
class DroidMetadataHandlerSpec extends ExternalServiceSpec {
  val droidHandler: BaseMetadataHandler = DroidMetadataHandler()
  val sharePointHandler: BaseMetadataHandler = SharePointMetadataHandler()
  ...
  "normaliseValues" should "normalise only specified property values" in {
    ...
    val sharePointFilePathJson = "/sites/Retail/Shared Documents/file1.txt".asJson
    val sharePointDateLastModifiedJson = "2025-07-03T09:19:47Z".asJson
    val someOtherJson = "some other json value".asJson

    droidHandler.normaliseValues("file_path", droidFilePathJson) shouldBe "Content/folder/file1.txt".asJson
    ...
    sharePointHandler.normaliseValues("file_path", sharePointFilePathJson) shouldBe "sites/Retail/Shared Documents/file1.txt".asJson
    sharePointHandler.normaliseValues("date_last_modified", sharePointDateLastModifiedJson) shouldBe "1751534387000".asJson
    sharePointHandler.normaliseValues("some_other_property", someOtherJson) shouldBe someOtherJson
  }
  ...
  "convertToBaseMetadata" should "convert valid Droid json with no default properties to base metadata json" in {
    ...
    val expectedJson = convertStringToJson(validBaseMetadataJsonString("Content/folder/file1.txt"))

    droidHandler.convertToBaseMetadata(rawSharePointJson) shouldBe expectedJson
  }

  "convertToBaseMetadata" should "convert valid Droid json with default properties to base metadata json" in {
    val rawDroidJsonString = """{
      | "SIZE": "12",
      | "LAST_MODIFIED": "2025-07-03T09:19:47",
      | "NAME": "file1.txt",
    ...
    val rawSharePointJson = convertStringToJson(rawDroidJsonString)
    val expectedJson = convertStringToJson(validBaseMetadataJsonString("Content/folder/file1.txt", closureType = "Closed"))
  ...
  "convertToBaseMetadata" should "convert valid SharePoint json with no default properties to base metadata json" in {
    ...
    val expectedJson = convertStringToJson(validBaseMetadataJsonString(filePath = "sites/Retail/Shared Documents/file1.txt"))

    sharePointHandler.convertToBaseMetadata(rawSharePointJson) shouldBe expectedJson
  }
  ...
  "convertToBaseMetadata" should "convert valid SharePoint json with default properties to base metadata json" in {
    ...
    val rawSharePointJson = convertStringToJson(rawSharePointJsonString)
    val expectedJson = convertStringToJson(validBaseMetadataJsonString(filePath = "sites/Retail/Shared Documents/file1.txt", closureType = "Closed"))
  ...
  "toClientSideMetadataInput" should "convert valid base metadata json to ClientSideMetadataInput" in {
    val input = droidHandler.toClientSideMetadataInput(convertStringToJson(validBaseMetadataJsonString(filePath = "Content/folder/file1.txt")))
  ...
  "toMetadataProperties" should "return specified properties if exist in json" in {
    ...
    val selectedMetadata = droidHandler.toMetadataProperties(convertStringToJson(validBaseMetadataJsonString("Content/folder/file1.txt")), properties)
