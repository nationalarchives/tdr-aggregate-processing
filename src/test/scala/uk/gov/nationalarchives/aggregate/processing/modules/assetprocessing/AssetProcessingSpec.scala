...
import org.scalatest.prop.{TableDrivenPropertyChecks, TableFor7, TableFor8}
...
class AssetProcessingSpec extends ExternalServiceSpec with TableDrivenPropertyChecks with MetadataHelper {
  ...
  val assetSources
      : TableFor8[String, (String, Long, UUID, Option[String], Option[String]) => String, String, String, String, String, List[MetadataProperty], List[MetadataProperty]] = Table(
    (
      "Source",
      "Default Metadata Json String",
      "Expected Parsed File Path",
      "Missing Required Field",
      "Supplied Metadata Json",
      "Custom Metadata Json",
      "Expected System Metadata",
      "Expected Supplied Metadata"
    ),
    (
      ...
      expectedSystemMetadata("content/Retail/Shared Documents/file1.txt"),
      expectedSuppliedMetadata
    ),
    (
      ...
      defaultSuppliedFields,
      customFieldJsonString,
      expectedSystemMetadata("top-level/Retail/Shared Documents/file1.txt"),
      expectedSuppliedMetadata
    ),
    (
      ...
      customFieldJsonString,
      expectedSystemMetadata("sites/Retail/Shared Documents/file1.txt") :+ MetadataProperty("date_created", "1751534387000"),
      List(MetadataProperty("description", "some kind of description"), MetadataProperty("closure status", "Open"))
    )
  )

  forAll(assetSources) {
    (assetSource, defaultMetadataJsonString, expectedParsedFilePath, missingRequiredField, suppliedFieldJson, customFieldJson, expectedSystemMetadata, expectedSuppliedMetadata) =>
      s"'processAsset' with asset source of $assetSource" should
        "return asset processing result and not log errors when metadata json is valid" in {
          ...
          val expectedResult = AssetProcessingResult(
            ...
            Some(expectedInput),
            systemMetadata = expectedSystemMetadata
          )
