...
class KeycloakClient(keycloakUtils: KeycloakUtils, config: Config)(implicit val executionContext: ExecutionContext) {
  ...
  private val readClientSecretPath = config.getString("auth.keycloakReadClientSecretPath")
  private val readSecret: String = getClientSecret(readClientSecretPath)
  ...
  def userDetails(userId: String): Future[UserDetails] = {
    ...
    keycloakUtils.userDetails(userId, "tdr-user-read", readSecret)
  }
}
